# 👋 Hey, I'm Ashish Rauniyar  
🚀 AI & Automation Engineer | Building Intelligent Automation Systems  

## 🧠 About Me  

I build **AI-powered systems** that automate workflows, reduce manual effort, and improve decision-making.  

- 🔹 Focused on **real-world AI applications**  
- 🔹 Building **production-ready AI + backend systems**  
- 🔹 Turning ideas into **scalable automation solutions**  

## 🤝 Let’s Connect  

<p>
  <a href="https://www.linkedin.com/in/ashish-rauniyar-6600961a8/">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin" />
  </a>

  <a href="https://www.youtube.com/@rauniai">
    <img src="https://img.shields.io/badge/YouTube-red?style=for-the-badge&logo=youtube" />
  </a>

  <a href="https://www.instagram.com/rauni.ai?igsh=d2h5eDlzdzFhbjcx">
    <img src="https://img.shields.io/badge/Instagram-purple?style=for-the-badge&logo=instagram" />
  </a>

  <a href="mailto:your-email@example.com">
    <img src="https://img.shields.io/badge/Email-black?style=for-the-badge&logo=gmail" />
  </a>
</p>
<details open>
<summary><b>⚙️ Technical Skills</b></summary>

<br>

### 🧩 AI / Machine Learning  
![LLM](https://img.shields.io/badge/LLMs-black?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG-blue?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-green?style=for-the-badge)
![LangGraph](https://img.shields.io/badge/LangGraph-purple?style=for-the-badge)

### 🛠 Backend & Development  
![Python](https://img.shields.io/badge/Python-yellow?style=for-the-badge&logo=python)
![FastAPI](https://img.shields.io/badge/FastAPI-teal?style=for-the-badge)
![API](https://img.shields.io/badge/REST_API-gray?style=for-the-badge)

### 🗄 Data & Storage  
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?style=for-the-badge&logo=postgresql)
![Supabase](https://img.shields.io/badge/Supabase-green?style=for-the-badge)
![VectorDB](https://img.shields.io/badge/Vector_DB-orange?style=for-the-badge)

### ☁️ Cloud & DevOps  
![Azure](https://img.shields.io/badge/Azure-blue?style=for-the-badge&logo=microsoftazure)
![Docker](https://img.shields.io/badge/Docker-blue?style=for-the-badge&logo=docker)
![Git](https://img.shields.io/badge/Git-black?style=for-the-badge&logo=git)

</details>

---

<details open>
<summary><b>🚀 What I Really Do</b></summary>

<br>

- 🤖 Build AI agents that can **plan, reason, and act**  
- ⚡ Automate workflows like **lead handling & operations**  
- 📊 Use data + AI for **intelligent decision-making**  
- 🔗 Combine **AI + backend + automation** into scalable systems  

</details>

---

<details open>
<summary><b>🔥 Projects working on</b></summary>

<br>

- 🧩 **AI Lead Management System**  
  Automates lead capture → qualification → response  

- 🤖 **AI Operations Agent**  
  Task planning, tool usage, memory-based execution  

- 📚 **AI Knowledge Chatbot (RAG)**  
  Context-aware answers from documents  

- 📊 **Cost Prediction Engine**  
  Predicts business costs using ML  

</details>

---

<details open>
<summary><b>🌟 Vision</b></summary>

<br>

To build an **AI-driven company** where systems:  

- Automate business operations  
- Reduce repetitive human work  
- Enable smarter, faster decision-making  


</details>



⭐ *Open to collaboration on AI systems, automation tools, and real-world problem solving.




